...
def _get_raw_daily_data(
    ...
    # get raw data 
    endpoints = ("daily_consumption", "daily_consumption_max_power")
    response = {}
    for endpoint in endpoints:
        ...
        if jsonfile.exists():
            response[endpoint] = __call_fake_enedis_api(api_tokens, start_date, end, endpoint, jsonfile=jsonfile)

        # if not, API is called
        else:
            # call api and get data
            response[endpoint] = __call_enedis_api(api_tokens, start_date, end, endpoint, jsonfile, write_json)
...
def _get_raw_load_curve(
    ...
    endpoints = ("consumption_load_curve", ) #, "production_load_curve")
    response = {f"{endpoint}":[] for endpoint in endpoints} 
    for endpoint in endpoints:
        print(f" >>  Endpoint : {endpoint}")
